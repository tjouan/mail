...
describe Mail::Sendmail do
  ...
  context 'SMTP From' do
    ...
    it 'escapes the From address' do
      ...
      described_class.should_receive :call do |_, arguments|
        arguments.should include('-f "\"from+suffix test\"@test.lindsaar.net"')
      end
      mail.deliver
    end
  end
  ...
  it 'escapes evil haxxor attempts' do
    ...
    described_class.should_receive :call do |_, arguments, destinations|
      arguments.should include("-f \"\\\"foo\\\\\\\"\\;touch /tmp/PWNED\\;\\\\\\\"\\\"@blah.com\"")
  ...
  it 'raise an error if no recipient if defined' do
    Mail.defaults do
      delivery_method :test
    end
